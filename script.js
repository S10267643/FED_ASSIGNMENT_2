document.addEventListener("DOMContentLoaded", function () {
    const APIKEY = "679717f9f9d2bbe70d181e2e"; // Your API Key
    ...
    document.getElementById("signUpForm").addEventListener("submit", async function (event) {
      ...
      try {
        const apiUrl = "https://mokesell-1e1c.restdb.io/rest/system_jobs"; // Replace with your DB URL
    ...
    // Handle Delete Account functionality (If required)
    document.getElementById("deleteAccountBtn")?.addEventListener("click", async function () {
      ...
      if (userId) {
        try {
          const apiUrl = `https://mokesell-1e1c.restdb.io/rest/system_jobs/${userId}`; // Update with your DB URL and user ID
          const response = await fetch(apiUrl, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
